...
const [userLocation, setUserLocation] = useState(null);
  ...
  const fetchApprovedPhotos = async () => {
    try {
      setLoading(true);
      // Fetch all approved photos with coordinates
      const response = await photosAPI.getPhotosByCoordinates({ 
        status: 'approved',
        limit: 1000 // Adjust based on your needs
      });

      console.log('Fetched photos for map:', response);
      
      // Filter photos that have valid coordinates
      const photosWithCoords = (response || []).filter(
      ...
      {photos.map((photo) => {
        ...
        return (
          <Marker
            key={photo._id}
            position={[lat, lng]}
            icon={createSinglePhotoMarker()}
          >
            <Popup>
              <div className="text-center p-2" style={{ minWidth: '200px' }}>
                {/* Photo thumbnail */}
                {photo.originalUrl && (
                  <img 
                    src={photo.originalUrl} 
                    alt={photo.placeName || 'Photo'} 
                    className="w-full h-32 object-cover rounded mb-2"
                  />
                )}
                
                {/* <h3 className="font-semibold text-sm mb-1">
                  {photo.placeName || 'Unknown Location'}
                </h3>
                 */}
                ...
                {photo.userId && (
                  <p className="text-xs text-gray-500 mb-2">
                    by {photo.userId.name || 'Anonymous'}
                  </p>
                )}

                <div className="flex items-center justify-center space-x-3 text-xs text-gray-500 mb-2">
                ...
                <button
                  ...
                  className="bg-blue-600 text-white px-4 py-1 rounded text-xs hover:bg-blue-700 transition w-full"
                >
                  View Full Photo
                </button>
              </div>
